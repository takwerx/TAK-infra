...
def generate_caddyfile(settings=None):
    ...
    if portal.get('installed'):
        ...
        if ak.get('installed'):
            ...
            lines.append(f"        }}")
            lines.append(f"")
            lines.append(f"        # Authenticated traffic to TAK Portal")
            lines.append(f"        reverse_proxy 127.0.0.1:3000")
            lines.append(f"    }}")
        else:
            lines.append(f"    reverse_proxy 127.0.0.1:3000")
