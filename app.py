...
"""
        ...
        try:
            ...
            if ak_token:
                ...
                plog("  Verifying bootstrap token access...")
                token_ok = False
                for attempt in range(12):  # up to 60s
                    try:
                        ...
                        token_ok = True
                        plog(f"  ✓ Bootstrap token verified")
                        break
                    except urllib.error.HTTPError as e:
                        if e.code == 403:
                            plog(f"  ⏳ Token not yet active (403)... waiting")
                            time.sleep(5)
                        else:
                            plog(f"  ⚠ Token check: {e.code}")
                            break
                    except Exception as e:
                ...
                if not token_ok:
                    plog("  ⚠ Bootstrap token not working — skipping admin setup")
