...
def wait_for_apt_lock(log_fn, timeout=1800):
    ...
    Waits indefinitely (up to 30 min), ticking a single time line every 10s.
    """
    ...
    while waited < timeout:
        ...
        if not r.stdout.strip():
            ...
            return True
        m, s = divmod(waited, 60)
        # Overwrite the last log line with updated time (same as TAK Server wait)
        if authentik_deploy_log and authentik_deploy_log[-1].startswith('[') and '⏳' in authentik_deploy_log[-1]:
            authentik_deploy_log[-1] = f"  ⏳ {m:02d}:{s:02d}"
        elif caddy_deploy_log and caddy_deploy_log[-1].startswith('[') and '⏳' in caddy_deploy_log[-1]:
            caddy_deploy_log[-1] = f"  ⏳ {m:02d}:{s:02d}"
        elif takportal_deploy_log and takportal_deploy_log[-1].startswith('[') and '⏳' in takportal_deploy_log[-1]:
            takportal_deploy_log[-1] = f"  ⏳ {m:02d}:{s:02d}"
        else:
